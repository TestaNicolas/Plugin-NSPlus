...
//     username: username,
//     userId: "1111",
//     userEmail: "alumno@mail.com",
//     userRole: "student",
//     courseName: "Materia Test",
//     fullname: "Fullname",
//     isExam: 0,
//     examStartDate: "", 
//     examEndDate: "",
...
(function() {
    /*
    NSPlus: Version encapsulada del editor de diagramas
    */
   //-------------------------------------------------
   /*!
...
function Enumeration() {
    ...
    function addEnumValue(key, value) {
        // -- It prevents the value overriding if the key already exists.
        _keys.push(key);
...
//---------------------------------------------
/* Extended Nassi Shneiderman definitions */
var base = {
	"declaration": {
		"class": "LaClase",
		"modifiers": "public",
...
function NSPPDF() {
	...
	this.course = document.getElementById("PDFCourse");
	...
	this.setProject = function (aProject, simpleFlag) {
		...
		this.course.innerHTML = aProject.getInfo("com");
	...
	this.watermarkStr = function () {
		return this.autor.innerHTML + "<br/>" + this.course.innerHTML + "<br/>" + this.date.innerHTML;
	}
}
//-----------------------------
/* - NSPFILE - */
...
function exportProject() {
    ...
    const filename = nsParams.username+"-"+nsParams.courseName+".nsplus";
...
function jsonToProject(projectJSON) {
	var newP = new NSPProject(nsParams);
...
function handleCheckObjects(e) {
	...
	if (e.target.checked) {
		methodName.innerHTML = "mÃ©todo";
	} else {
		methodName.innerHTML = "funciÃ³n";
	}
}
...
function NSPProject(data) {
	var _self = this;
	var _DEFAULT_NAME = "Proyecto sin tÃ­tulo";
	var _DEFAULT_AUTHOR = "Sin autor";
	var _DEFAULT_GROUP = "Sin curso";
	...
	var _env = {
		"tea": false,
		"uid": nsParams.userId,
		"evs": nsParams.examStartDate,
		"eve": nsParams.examEndDate,
		"usr": nsParams.username,
		"com": nsParams.courseName,
		"notify": null
	}
	var _defaults = {
		"usr": _DEFAULT_AUTHOR,
	...
	function init() { resetData(); setData(data); setEnv(data); starCheckTimer() }
...
function reSize() {
	...
	diagramContainer.container.style.height = newSectionDiagramHeight - paddingTopSection - paddingBottomSection;
}
...
function va(x) {
	return (function(z){
		return _validator.validate(z)})(nsParams.username);
}

function par() {
	nsParams.notify = eventProc;
	return nsParams;
}
...
function checkOrigin() {
	function c() { return nsParams.username && nsParams.courseName }
	function b() { return nsParams.isExam == 1 }
	show();
}
